...
def write_report(by_lib: dict[str, dict]) -> None:
    ...
    lines: list[str] = []
    cj_idx = index_results(by_lib["fastxml"])
    ...
    lines.append("")
    lines.append("## 测试期间对 fastxml 库的 bug 排查")
    lines.append("")
    ...
    lines.append("DOM / parser / writer / escape 层面的算法优化已经基本榨干"
                 ...
                 " fastxml 算法层，而落在仓颉编译器、运行时与标准库的通用"
                 "性能特性上。**")
    lines.append("")
    ...
    lines.append("详见仓库根的 `problem.md`。这些项的修复或公开 API 暴露能让 "
                 "fastxml 进一步逼近 tinyxml2 量级，且会同时受益于其它字符密集"
                 "型库（JSON / TOML / 协议解析等）。")
    lines.append("")
