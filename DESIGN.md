...
> 面向仓颉语言，对 `.tinyxml2-11.0.0` 的 **功能、边界与工程经验** 做系统吸收，产出一套更符合仓颉语言气质的 XML 库设计方案。
>
> 这不是把 tinyxml2 从 C++ 逐行翻译到仓颉，而是要在保持 XML 语义、DOM 能力和 tinyxml2 实用主义优势的前提下，借助仓颉的 `enum`、模式匹配、`Option`、扩展、泛型、并发和规范化包结构，设计一套 **结构清晰、命名优雅、职责对称、可渐进实现** 的纯仓颉 XML 库。
...
本文与 [`ROADMAP.md`](./ROADMAP.md) 配套：
- `DESIGN.md` 解决“**做成什么样**”；
- `ROADMAP.md` 解决“**按什么节奏做出来**”。
