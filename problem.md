# fastxml 性能调优中观察到的仓颉编译器 / 运行时 / 标准库低效问题

> **背景**：在把 `fastxml` 在算法 / 数据结构层面优化到与 tinyxml2 同型实现
...
> 建议"五段式整理，旨在反馈给 Cangjie SDK 团队；与 `fastxml` 库本身的
> bug 无关。
>
...
| 维度 | 库层（fastxml）已榨干 | 等价于"等 SDK 改进" |
|---|---|---|
...
**结论**：常见的"算法 / 数据结构"维度优化已经做满；继续把 fastxml 推到
