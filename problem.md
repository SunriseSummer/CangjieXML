# tinyxml2 性能调优中观察到的仓颉编译器 / 运行时 / 标准库低效问题

> **背景**：在把 `tinyxml2` 在算法 / 数据结构层面优化到与 tinyxml2 同型实现
> 后，相对 tinyxml2 的剩余倍率仍稳定在 6~35×。继续往下挖掘后发现，**剩余
> 差距并非来自库自身的算法选择，而是落在仓颉通用基础设施上**——同一类
> 问题会同时拖慢 JSON / TOML / 协议解析 / 字符串模板渲染等任何字符密集型库。
>
> 本文档把这些观察按"症状 → 复现路径 → 已尝试 workaround → 影响估算 → 改进
> 建议"五段式整理，旨在反馈给 Cangjie SDK 团队；与 `tinyxml2` 库本身的
> bug 无关。
>
...
| 维度 | 库层（tinyxml2）已榨干 | 等价于"等 SDK 改进" |
|---|---|---|
...
**结论**：常见的"算法 / 数据结构"维度优化已经做满；继续把 tinyxml2 推到
