...
1. **先稳定结构，再丰富能力。**
2. **先完成写出层，再实现解析层。** 这样可以尽早建立字符串黄金测试。
...
### 风险
- 需要优先验证仓颉 SDK `1.0.5` 在 GitHub Actions Linux x64 环境中的下载、解压、路径配置与 `cjpm` 可执行链路是否稳定。
- `cjpm.toml` 初版应至少为 `name`、`version`、`cjc-version`、`output-type`、后续测试入口和示例入口预留可演进空间，避免 M1 之后再次改动项目骨架。
...
### 交付物
- `src/tinyxml2/writer/xml_write_options.cj`
- `src/tinyxml2/writer/xml_writer.cj`
- `src/tinyxml2/internal/text_escape.cj`
...
### 交付物
- `src/tinyxml2/parser/xml_parse_options.cj`
- `src/tinyxml2/parser/xml_source.cj`
- `src/tinyxml2/parser/xml_parser.cj`
- `src/tinyxml2/internal/slice.cj`
- `src/tinyxml2/internal/normalized_buffer.cj`
- `src/tinyxml2/internal/entity_decoder.cj`
...
### 交付物
- `src/tinyxml2/query/xml_element_query_ext.cj`
- `src/tinyxml2/query/xml_node_iter_ext.cj`
- `src/tinyxml2/query/xml_find_ext.cj`
- `src/tinyxml2/build/xml_document_builder.cj`
- `src/tinyxml2/build/xml_element_builder.cj`
...
### 交付物
- `src/tinyxml2/visit/xml_visitor.cj`
- `src/tinyxml2/visit/xml_walk.cj`
...
### 交付物
- `src/tinyxml2/error/source_pos.cj`
- `src/tinyxml2/error/xml_error.cj`
- `src/tinyxml2/io/xml_loader.cj`
- `src/tinyxml2/io/xml_saver.cj`
...
### 交付物
- `src/tinyxml2/batch/xml_batch_parser.cj`（如决定采用独立包）
- 并发测试与示例
