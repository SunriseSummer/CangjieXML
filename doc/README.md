...
| [03 — DOM](./03-dom.md)                      | `fastxml.dom`         | 节点模型、文档 / 元素 / 属性         |
| [04 — Parser](./04-parser.md)                | `fastxml.parser`      | 解析入口、选项、错误                 |
| [05 — Writer](./05-writer.md)                | `fastxml.writer`      | 序列化、`XmlWriteOptions`、`XmlSink` |
| [06 — Query](./06-query.md)                  | `fastxml.query`       | 类型化属性 / 子元素查询，codec       |
| [07 — Builder](./07-builder.md)              | `fastxml.build`       | `XmlDocumentBuilder` DSL             |
| [08 — Visit](./08-visit.md)                  | `fastxml.visit`       | `walk` 与 `XmlVisitor`               |
| [09 — IO](./09-io.md)                        | `fastxml.io`          | 文件 / 字节读写、流式 Sink           |
| [10 — 错误模型](./10-errors.md)              | `fastxml.error` 等    | 异常体系与错误分层                   |
...
- 文档中绝大多数仓颉代码都在 `src/doc_examples/doc_examples_test.cj` 里被
  当作 `@Test` 跑过一遍——示例构造的 DOM 与断言都与生产代码同源。
- `cjpm test` 绿灯等价于"文档中的可运行示例全部可编译、可运行、结果符合文档描述"。
- 极少量"示意草图"（例如用户扩展点的雏形）不会进入自动化验证，对应章节会
  明确指出。
- API 演化时，任何破坏性修改都必须同步更新本目录下对应章节。

如果你在文档里看到"例如……"而示例没有被测试文件覆盖，请视为 bug 并提 issue。
